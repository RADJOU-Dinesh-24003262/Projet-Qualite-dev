...
class AbstractCharacterTest {
    ...
    @Test
    @DisplayName("EatFood: BUG - Logique inversée pour nourriture fraîche/non-comestible")
    void testEatFoodBugWithLogic() {
        // Ce test documente le bug actuel dans le code
        attacker.setHealth(100);
        int initialHealth = attacker.getHealth();
        
        attacker.eatFood(boar);
        
        // BUG: La condition est inversée et devrait faire perdre 10 HP
        // pour nourriture non-fraîche ou non-comestible
        assertTrue(attacker.getHealth() <= initialHealth);
    }

    @Test
    @DisplayName("EatFood: BUG - setHunger utilise getHealth au lieu de getHunger")
    void testEatFoodBugWithHunger() {
        ...
        // BUG: hunger devrait être 30 + nutritionalScore
        // mais le code utilise getHealth() au lieu de getHunger()
        assertNotEquals(30 + boar.getNutritionalScore(), attacker.getHunger());
    }
